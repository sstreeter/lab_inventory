...
def import_csv():
    if request.method == 'POST':
        mode = request.form.get('mode')  # overwrite, merge, retain, revert
        ...
        with open(filepath, newline='') as csvfile:
            ...
            reader.fieldnames = [field.strip() for field in reader.fieldnames]  # Normalize header names
            for row in reader:
                lab_name = row.get("Lab", "").strip()
                # Use the first available identifier type in order of precedence
                identifier = ""
...
def resolve_conflicts():
    ...
    if request.method == 'POST':
        for c in conflicts:
            ...
            comp = Computer.query.filter_by(lab_id=lab.id, identifier=c['identifier']).first()
